...
namespace SANSurveyWebAPI.Areas.Admin.Controllers
{
    ...
    public class ProfileController : BaseController
    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            for (var row = 2; row <= lastRow; row++)
                            {
                                ...
                                s.ClientName = worksheet.Cells[row, 6].Value.ToString();
                                s.ClientInitials = worksheet.Cells[row, 7].Value.ToString();
        ...
        public async Task<ActionResult> Create([Bind(Include = "Name,MobileNumber,LoginEmail,OffSetFromUTC,Incentive,ClientName")] Profile profile)
        {
            if (ModelState.IsValid)
            {
                ...
                if (profile.ClientName == "Junior Doctors")
                    profile.ClientInitials = "JD";
                else if (profile.ClientName == "Warren and Mahoney")
                    profile.ClientInitials = "WAM";
                else if (profile.ClientInitials == "Case Workers")
                    profile.ClientInitials = "CW";
        ...
        public async Task<ActionResult> InviteEmail(int? id)
        {
            ...
            if (k != null)
            {
                ...
                if (k.ClientInitials != null || k.ClientInitials != "")
                {
                    if (k.ClientInitials.ToLower().ToString() == "wam")
                    {
                        e.Link = GetBaseURL() + "Register/Guide" + "/" + uid; //For Warren and Mahony
                    }
                    else
                    {
                        e.Link = GetBaseURL() + "Register/Index" + "/" + uid; //For Junior Doctors
                    }
                }
                ...
                await Task.Run(() =>
                {
                    if (k.ClientInitials.ToLower().ToString() == "jd")
                    { PostalEmail.RegisterAndBaseline(e); }
                    else if (k.ClientInitials.ToLower().ToString() == "wam")
                    { PostalEmail.WarrenMahonyBaseline(e); }
                });
            }
            else
            {
        ...
        #endregion
        #region Reminder Email No Hangfire
        public async Task<ActionResult> ReminderEmail(int? id)
        {
            if (!id.HasValue)
            {
                return new System.Web.Mvc.HttpStatusCodeResult(HttpStatusCode.BadRequest);
            ...
            if (k != null)
            {
                ...
                {
                    if (k.ClientInitials.ToLower().ToString() == "jd")
                    { e.Link = GetBaseURL() + "Register/Index" + "/" + k.Uid; }
                    else if (k.ClientInitials.ToLower().ToString() == "wam")
                    { e.Link = GetBaseURL() + "Register/Guide" + "/" + k.Uid; }
                }
                else if (k.MaxStep == 1 && k.RegistrationProgressNext == "Signup")
                {
                    if (k.ClientInitials.ToLower().ToString() == "jd")
                    { e.Link = GetBaseURL() + "Account/Signup" + "?" + k.Uid; }
                    else if (k.ClientInitials.ToLower().ToString() == "wam")
                    { e.Link = GetBaseURL() + "Account/Signup" + "?uid=" + k.Uid; }
                }
                else
                {
                    e.Link = GetBaseURL() + "Account/Login";
                }
                e.Incentive = k.Incentive;                
                ...
                {
                    if (k.ClientInitials.ToLower().ToString() == "jd")
                    { PostalEmail.ReminderBaseline(e); }
                    else if (k.ClientInitials.ToLower().ToString() == "wam")
                    { PostalEmail.WarenMahonyReminderBaseline(e); }
                });
            }
            else
            {
